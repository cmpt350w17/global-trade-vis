...
(function() {
	...
	var QuickSettings = {
		...
		/**
		* Repositions the pannel according to the given offset
		* @param x 		{Number} Amount to change the x position by.
		* @param y 		{Number} Amount to change the y position by.
		* @returns 	{module:QuickSettings}
		*/
		movePosition: function(x,y){
			console.log(this._panel.style.left);
			//this._panel.style.left = (this._panel.style.left + x) + "px";
		...
		addBoolean: function(title, value, callback) {
			...
			input.addEventListener("change", function() {
				if(callback) {
					//// THIS LINE WAS CHANGED TO GET THE TITLE
					callback(input.title, input.value);
				}
				self._callGCH(title);
			});
			return this;
